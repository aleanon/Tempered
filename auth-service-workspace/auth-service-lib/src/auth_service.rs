...
impl AuthService {
    ...
    ///
    /// # Arguments
    /// * `user_store` - Store for user data
    /// * `banned_token_store` - Store for banned JWT tokens
    /// * `two_fa_code_store` - Store for 2FA codes
    /// * `email_client` - Client for sending emails
    ///
    /// # Note on Architecture
    /// Due to Axum's routing constraints, each route is added with its specific state requirements.
    /// While this means calling `.with_state()` multiple times, it's more efficient than wrapping
    /// handlers since it avoids extra function call overhead at runtime.
    pub fn new<U, B, T, E>(
        user_store: U,
    ...
    /// Build the router with all auth routes
    ///
    /// Each route is paired with the specific state it needs. This approach:
    /// - Avoids unnecessary state cloning for routes that don't need all stores
    /// - Makes dependencies explicit at the route level
    /// - Maintains type safety without runtime overhead
    pub fn with_state<U, B, T, E>(
