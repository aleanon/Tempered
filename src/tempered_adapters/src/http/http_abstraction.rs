...
//! on their own types, avoiding any allocation or copying overhead.
//!
//! # Architecture
//!
//! ```text
//! ┌─────────────────────────────────────────┐
//! │  axum::Request implements AuthRequest   │  ← Zero cost!
//! │  (no copying, just method delegation)   │
//! └──────────────┬──────────────────────────┘
//!                │
//!                ▼
//! ┌──────────────────────────────────────────┐
...
//!
//! ```ignore
//! // Axum implements the trait on its request type - zero allocations!
...
///
/// Web frameworks implement this trait on their request types to enable
/// authentication without any allocation or copying overhead.
///
...
///
/// ```ignore
/// impl AuthRequest for axum::http::Request<axum::body::Body> {
///     fn header(&self, name: &str) -> Option<&str> {
///         self.headers().get(name)?.to_str().ok()
///     }
///
...
///
/// Web frameworks implement this trait on their response builder types
/// to enable authentication responses without allocations.
///
/// # Design
///
...
///
/// ```ignore
/// impl AuthResponseBuilder for axum::http::response::Builder {
...
///         self.body(axum::body::Body::empty()).unwrap()
///     }
/// }
/// ```
